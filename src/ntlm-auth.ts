...
function axiosDataToString(data: unknown): string {
...
// azure-devops-node-api hands `message` straight back to callers for stream APIs
// (getItemText, getAttachmentContent), so it must be a real readable stream and
// not just a status/header bag.
function createHttpClientResponse(response: AxiosResponse<string>): NtlmHttpClientResponse {
  const body = axiosDataToString(response.data);
  ...
  message.headers = response.headers as http.IncomingHttpHeaders;
...
function axiosErrorToFetchResponse(error: unknown): Response {
  if (typeof error === "object" && error !== null && "response" in error) {
    const axiosError = error as { response?: AxiosResponse<string> };
    if (axiosError.response) {
      return new Response(axiosDataToString(axiosError.response.data), {
        status: axiosError.response.status,
...
export function installNtlmFetchInterceptorFromContext(getCredentials: () => NtlmCredentials | undefined): void {
  ...
  globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    try {
      ...
      return new Response(axiosDataToString(response.data), {
        status: response.status,
